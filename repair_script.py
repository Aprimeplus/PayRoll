...
print("Starting repair script...")

# The missing functions block
missing_code = r'''
...
try:
    ...
    for i, line in enumerate(lines):
        if "def delete_company_holiday" in line:
            start_index = i
            print(f"Found start marker at line {i}: {line.strip()}")
            break
            
    end_index = -1
    for i in range(len(lines)):
        if "def get_dashboard_stats" in line: # Wait, I used 'line' from the previous loop! BUG!
             pass # I will fix this in the next write.
        if "def get_dashboard_stats" in lines[i]:
            end_index = i
    ...
    if start_index != -1 and end_index != -1:
        ...
        part3 = lines[end_index:]
        
        # Correct delete_company_holiday
        delete_holiday_code = r'''def delete_company_holiday(holiday_id):
    """ลบวันหยุดจาก ID"""
    conn = get_db_connection()
    if not conn: return False
    try:
...
'''
        ...
        # update_user_password code
        update_password_code = r'''
...
'''
        ...
        new_content = "".join(part1) + delete_holiday_code + "\n" + missing_code + "\n" + update_password_code + "\n" + "".join(part3)
        
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
            
        print("Successfully repaired hr_database.py")
    else:
        print("Could not find start/end markers.")
        # Debug: print some lines around where they should be
        print("Searching for 'def delete_company_holiday'...")
        for i, line in enumerate(lines):
            if "delete_company_holiday" in line:
                print(f"{i}: {line.strip()}")
        
        print("Searching for 'def get_dashboard_stats'...")
        for i, line in enumerate(lines):
            if "get_dashboard_stats" in line:
                print(f"{i}: {line.strip()}")
